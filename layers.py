...
class BatchNormalization(Module):
    ...
    def compute_norm_input(self, input: np.array, 
                           mean: np.array, var: np.array) -> np.array:
        """
        :param input: array of shape (batch_size, num_features)
        :return: array of shape (batch_size, num_features)
        """
        inv_sqrt_var = np.reciprocal(np.sqrt(var + self.eps))
        norm_input = (input - mean) * inv_sqrt_var
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        if self.training:
            ...
            self.var = (self.input_mean ** 2).mean(axis=0)
            self.inv_var = np.reciprocal(self.var + self.eps)
            self.sqrt_var = np.sqrt(self.var)
            self.inv_sqrt_var = np.sqrt(self.inv_var)
            self.norm_input = self.input_mean * self.inv_sqrt_var
            if self.affine:
                self.output = self.norm_input * self.weight + self.bias
            else:
            ...
            self.running_var = (1 - self.momentum) * self.running_var \
                                + self.momentum * self.var * batch_size / float(batch_size - 1)

        else:
            norm_input = self.compute_norm_input(input, self.running_mean, self.running_var)
            if self.affine:
                self.output = norm_input * self.weight + self.bias
            else:
                self.output = norm_input
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        if self.training:
            ...
            var = ((input - mean) ** 2).mean(axis=0)
            inv_sqrt_var = np.reciprocal(np.sqrt(var + self.eps))
            norm_input = self.compute_norm_input(input, mean, var)
            
            grad_input = (1. / batch_size) * inv_sqrt_var \
                * (batch_size * grad_norm_input - np.sum(grad_norm_input, axis=0) \
                - norm_input * np.sum(grad_norm_input * norm_input, axis=0))
            
            return grad_input
        
        else:
            inv_sqrt_running_var = np.reciprocal(np.sqrt(self.running_var + self.eps))
            grad_input = grad_norm_input * inv_sqrt_running_var
            return grad_input
    ...
    def update_grad_parameters(self, input: np.array, grad_output: np.array):
        ...
        grad_weight = (self.compute_norm_input(input, mean, var) * grad_output).sum(axis=0)
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        if self.training:
            ...
            self.output = self.mask * input / float(1 - self.p)
        else:
            self.output = input
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        if self.training:
            return grad_output * self.mask / float(1 - self.p)
        else:
            return grad_output
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        for module in self.modules:
            cur_input = module.forward(cur_input)
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        for i in range(len(self.modules) - 1, 0, -1):
            cur_grad_output = self.modules[i].backward(self.modules[i - 1].output, cur_grad_output)
        grad_input = self.modules[0].backward(input, cur_grad_output)
