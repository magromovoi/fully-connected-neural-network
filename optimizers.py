...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            """
            np.add(0, self.momentum * m + (grad + self.weight_decay * param), out=m)
            np.add(0, param - self.lr * m, out=param)
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            hint: consider using np.add(..., out=m) for in place addition,
              i.e. we need to change original array, not its copy
            """
            grad_weight_decay = (grad + self.weight_decay * param)
            np.add(0, self.beta1 * m + (1 - self.beta1) * grad_weight_decay, out=m)
            np.add(0, self.beta2 * v + (1 - self.beta2) * grad_weight_decay ** 2, out=v)
            np.add(0, param - self.lr * m / (1 - self.beta1 ** t) \
                   / (np.sqrt(v / (1 - self.beta2 ** t)) + self.eps), out=param)
